...
6. Multiplayer uses a Supabase Realtime channel named `tbpn-sim:world:<room>` with Presence for room membership and broadcast events for live movement updates. The browser now joins that room as a private authenticated channel, so the `realtime.messages` policies in the latest schema are required.
